...
# Top-level config keys needed for non-streaming inference. Everything
# else (GAN discriminators, dataset processor pipelines, training conf)
# pulls in heavy dependencies (matcha.hifigan, pyarrow, pyworld) that
# inference never uses, so strip those keys before loading.
_INFERENCE_CONFIG_KEYS = {
    ...
    'chunk_size', 'num_decoding_left_chunks', 'speech_token_size',
    'llm', 'flow', 'hift', 'get_tokenizer', 'allowed_special',
    'feat_extractor',
}


def _load_inference_configs(hyper_yaml_path, overrides=None):
    """Load a CosyVoice yaml config, keeping only inference keys."""
    import io
    import re as _re
    with open(hyper_yaml_path, 'r') as f:
        text = f.read()
    top_keys = _re.findall(r'^([A-Za-z_][A-Za-z0-9_]*):', text, _re.M)
    for key in top_keys:
        if key not in _INFERENCE_CONFIG_KEYS:
            text = _re.sub(
                r'^{}:.*?(?=^[A-Za-z_][A-Za-z0-9_]*:|\Z)'.format(key),
                '', text, flags=_re.M | _re.S)
    return load_hyperpyyaml(io.StringIO(text), overrides=overrides)
    ...
    def __init__(self, model_dir, load_jit=False, load_trt=False, fp16=False, trt_concurrent=1):
        ...
        if not os.path.exists(model_dir):
            raise FileNotFoundError(
                'model_dir {} does not exist: download the model first '
                '(e.g. via huggingface_hub.snapshot_download)'.format(model_dir))
        hyper_yaml_path = '{}/cosyvoice.yaml'.format(model_dir)
        if not os.path.exists(hyper_yaml_path):
            raise ValueError('{} not found!'.format(hyper_yaml_path))
        configs = _load_inference_configs(hyper_yaml_path)
...
class CosyVoice2(CosyVoice):
    ...
    def __init__(self, model_dir, load_jit=False, load_trt=False, load_vllm=False, fp16=False, trt_concurrent=1):
        ...
        if not os.path.exists(hyper_yaml_path):
            raise ValueError('{} not found!'.format(hyper_yaml_path))
        configs = _load_inference_configs(
            hyper_yaml_path,
...
def AutoModel(**kwargs):
    if not os.path.exists(kwargs['model_dir']):
        raise FileNotFoundError(
            'model_dir {} does not exist: download the model first '
            '(e.g. via huggingface_hub.snapshot_download)'.format(kwargs['model_dir']))
