...
def load_wav(wav, target_sr, min_sr=16000):
    # torchaudio 2.11 dropped backend='soundfile' in favor of torchcodec;
    # read via soundfile directly to avoid that extra native dependency.
    import soundfile as sf
    data, sample_rate = sf.read(wav, dtype='float32')
    speech = torch.from_numpy(data)
    if speech.dim() == 2:
        speech = speech.transpose(0, 1).mean(dim=0, keepdim=True)
    else:
