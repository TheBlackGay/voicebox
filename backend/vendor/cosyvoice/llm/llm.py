...
class Qwen2Encoder(torch.nn.Module):
    ...
    def forward_one_step(self, xs, masks, cache=None):
        # transformers >= 4.5x pads a short 2D attention_mask with zeros when a
        # past_key_values cache is present (prepare_padding_mask), which would
        # mask out every cached position and leave only the first token
        # attendable. Build a full-length all-ones mask instead so the model can
        # attend to the whole prefix during step-by-step decoding.
        if cache is not None:
            past_len = cache.get_seq_length()
            input_masks = torch.ones((xs.shape[0], xs.shape[1] + past_len), dtype=torch.bool, device=xs.device)
        else:
            input_masks = masks[:, -1, :]
        outs = self.model(
            inputs_embeds=xs,
